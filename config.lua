...
frame:SetScript("OnShow", function(frame)
	...
	filterLabel:SetPoint("TOPLEFT", clear, "BOTTOMLEFT", 0, -25)
	filterLabel:SetText("Ignore errors matching this expression")

	local filterExpressionBox = CreateFrame("EditBox", nil, frame, "InputBoxTemplate")
	filterExpressionBox:SetAutoFocus(false)
	filterExpressionBox:SetPoint("LEFT", filterLabel, "RIGHT", 16, 0)
	filterExpressionBox:SetPoint("RIGHT", -25, 0)
	filterExpressionBox:SetHeight(20)
	filterExpressionBox:SetScript("OnEnterPressed", filterExpressionBox.ClearFocus)
	filterExpressionBox:SetText(addon.db.filterExpression or "")
	filterExpressionBox:SetScript("OnTextChanged", function(self, text)
		addon.db.filterExpression = self:GetText()
	end)

	local filterLabelExample = frame:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
	filterLabelExample:SetPoint("TOPLEFT", filterLabel, "BOTTOMLEFT", 0, -3)
	filterLabelExample:SetTextColor(1, 1, 1, 0.5)
	filterLabelExample:SetText("Example: RXPGuides.+TargetUnit")
